...
type
  TfrmGerar_Lote_Calc = class(TForm)
    ...
    procedure ListaConsultada1Click(Sender: TObject);
  private
...
procedure TfrmGerar_Lote_Calc.ListaConsultada1Click(Sender: TObject);
var
  Arq: string;
begin
  Arq := ExtractFilePath(Application.ExeName) + 'Relatorios\Lote_Lista.fr3';
  if not (FileExists(Arq)) then
  begin
    MessageDlg('*** Relatório não localizado: ' + Arq , mtInformation, [mbOk], 0);
    exit;
  end;
  fDMCadLote_Calc.frxReport1.Report.LoadFromFile(Arq);
